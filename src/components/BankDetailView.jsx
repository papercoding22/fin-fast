...
function SummaryCard({ label, value, sub, accent, warn }) {
  ...
  if (accent) { bg = 'bg-blue-50 border-blue-200'; textColor = 'text-blue-700'; }
  if (warn) { bg = 'bg-amber-50 border-amber-200'; textColor = 'text-amber-800'; }
  return (
  ...
  if (mode === 'direct') {
    return (
      <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
        ...
        <p className="text-xs text-slate-400 mt-1">Nhập trực tiếp — không liên kết chỉ số tham chiếu</p>
    ...
    <div className="p-4 bg-blue-50 rounded-xl border border-blue-200">
      ...
      {ref ? (
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
          <div><span className="text-blue-600">Chỉ số tham chiếu:</span> <span className="font-medium text-slate-800">{ref.name}</span></div>
          <div><span className="text-blue-600">Ngân hàng phát hành:</span> <span className="font-medium text-slate-800">{ref.provider}</span></div>
          <div><span className="text-blue-600">Giá trị hiện tại:</span> <span className="font-semibold text-slate-900">{ref.currentValue}%/năm</span></div>
          <div><span className="text-blue-600">Ngày hiệu lực:</span> <span className="font-medium text-slate-800">{ref.effectiveDate}</span></div>
          <div><span className="text-blue-600">Tần suất điều chỉnh:</span> <span className="font-medium text-slate-800">{ref.adjustmentFrequency}</span></div>
          <div><span className="text-blue-600">Biên độ cộng thêm:</span> <span className="font-medium text-slate-800">+{bank.spread}%</span></div>
          {ref.notes && (
            <div className="col-span-2 mt-1 text-slate-500 italic">{ref.notes}</div>
          )}
        </div>
      ) : (
      ...
      <p className="text-xs text-orange-600 mt-3 font-medium">
        ⚠ Lãi suất thả nổi sẽ được ngân hàng điều chỉnh định kỳ theo {ref?.adjustmentFrequency ?? '—'} — số liệu chỉ mang tính tham khảo tại thời điểm hiện tại.
      </p>
    </div>
  );
}
...
function StressTestSection({ bank, loan, additionalCosts, referenceIndexes }) {
  ...
  const stressResults = useMemo(() => {
    return STRESS_OFFSETS.map(offset => {
      ...
      const fullTermResult = calcScenario(schedule, loan.loanTermMonths, bank.prepaymentFees, additionalCosts);
      const settle36 = calcScenario(schedule, Math.min(36, loan.loanTermMonths), bank.prepaymentFees, additionalCosts);
  ...
  const indexLabel = mode === 'formula' && ref
    ? `${ref.name} (hiện tại ${ref.currentValue}%)`
    : 'Lãi suất thả nổi';
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              ...
              <th className="text-right px-4 py-3 font-medium text-slate-600 whitespace-nowrap">Lãi sau ưu đãi</th>
              <th className="text-right px-4 py-3 font-medium text-slate-600 whitespace-nowrap">Tổng lãi (hết hạn)</th>
              <th className="text-right px-4 py-3 font-medium text-slate-600 whitespace-nowrap">Chi phí ngoài gốc (hết hạn)</th>
              <th className="text-right px-4 py-3 font-medium text-slate-600 whitespace-nowrap">Chênh lệch vs hiện tại</th>
              <th className="text-right px-4 py-3 font-medium text-slate-600 whitespace-nowrap">Tổng lãi (36 tháng)</th>
            </tr>
          </thead>
          <tbody>
            {stressResults.map(({ offset, floatingRate, fullTermResult, settle36 }) => {
              ...
              const diff = baseResult
                ? fullTermResult.costExcludingPrincipal - baseResult.fullTermResult.costExcludingPrincipal
                : 0;
                ...
                <tr key={offset} className={`border-b border-slate-100 ${rowBg}`}>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      {isBase && <span className="text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full">Hiện tại</span>}
                      <span className="text-slate-700 font-medium text-xs">{label}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-right font-semibold">
                    <span className={offset > 0 ? 'text-red-700' : offset < 0 ? 'text-green-700' : 'text-blue-700'}>
                      {floatingRate.toFixed(2)}%/năm
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right text-slate-700">{formatVND(fullTermResult.totalInterestPaid)}</td>
                  <td className="px-4 py-3 text-right font-semibold text-slate-800">{formatVND(fullTermResult.costExcludingPrincipal)}</td>
                  ...
                  <td className="px-4 py-3 text-right text-slate-600">{formatVND(settle36.totalInterestPaid)}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="px-6 py-3 bg-orange-50 border-t border-orange-100">
        <p className="text-xs text-orange-700">
          ⚠ Stress test giả định lãi suất thay đổi ngay từ tháng ưu đãi kết thúc và giữ nguyên đến hết kỳ hạn.
          Thực tế lãi suất có thể biến động nhiều lần trong suốt thời gian vay.
        </p>
      </div>
    </div>
  );
...
function AmortizationTable({ schedule, bank, referenceIndexes }) {
  ...
  function downloadCSV() {
    ...
    const ref = referenceIndexes?.find(r => r.id === bank.refIndexId);
    const meta = bank.floatingRateMode === 'formula' && ref
      ? {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      <div className="mb-6">
        ...
        {result.isFullTerm && (
          <p className="mt-2 text-xs text-green-600 font-medium">Tất toán đúng hạn — không phát sinh phí phạt.</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <div className="p-4 bg-blue-50 rounded-xl border border-blue-200">
          ...
          <p className="text-xs text-blue-600 mt-1">Chi phí ngoài gốc: {formatVND(result.costExcludingPrincipal)}</p>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold" style={{ color: bank.color }}>{bank.name}</h2>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        {/* Loan overview */}
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 p-5" style={{ borderLeft: `4px solid ${bank.color}` }}>
            ...
            <div className="mt-3 pt-3 border-t border-slate-100">
              ...
              <div className="flex flex-wrap gap-2">
                {milestones.map(m => {
                  ...
                  return (
                    <div key={m} className="text-xs bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1">
                      <span className="text-slate-400">T{m}: </span>
                      <span className="font-medium text-slate-700">{formatVND(entry.remainingAfter)}</span>
                    </div>
                  );
                })}
              </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <SummaryCard label="Số tiền vay" value={formatVND(loan.loanAmount)} sub="Gốc cần hoàn trả" />
        <SummaryCard label="Tổng lãi (hết kỳ hạn)" value={formatVND(fullTermResult.totalInterestPaid)} accent />
        <SummaryCard label="Chi phí phụ" value={fullTermResult.totalAdditional > 0 ? formatVND(fullTermResult.totalAdditional) : 'Không có'} />
        <SummaryCard label="Chi phí ngoài gốc" value={formatVND(fullTermResult.costExcludingPrincipal)} accent sub="Lãi + phí + chi phí phụ" />
        ...
        <SummaryCard label="Trả tháng cuối" value={formatVND(lastPayment)} sub={`Lãi suất ${bank.floatingRate.toFixed(2)}%/năm`} />
        <SummaryCard
