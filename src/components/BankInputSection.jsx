...
export default function BankInputSection({
  banks,
  loan,
  onChange,
  onAdd,
  ...
  function removePrepaymentFee(id, feeIndex) {
    onChange(
      banks.map((b) => {
        ...
        return {
          ...b,
          prepaymentFees: b.prepaymentFees.filter((_, i) => i !== feeIndex),
        };
      }),
    );
  }
  ...
  function addPrepaymentFee(id) {
    onChange(
      banks.map((b) => {
        ...
        return {
          ...b,
          prepaymentFees: [...b.prepaymentFees, { upToYear: 999, rate: 0 }],
        };
      }),
    );
  }
  ...
  function updateAdditionalCost(id, costIndex, newCost) {
    onChange(
      banks.map((b) => {
        ...
        let cost = { ...newCost };
        if (cost.type === "percent_car")
          cost.amount = (loan.carPrice * cost.percent) / 100;
        if (cost.type === "percent_loan")
          cost.amount = (loan.loanAmount * cost.percent) / 100;
  ...
  function removeAdditionalCost(id, costIndex) {
    onChange(
      banks.map((b) => {
        ...
        return {
          ...b,
          additionalCosts: b.additionalCosts.filter((_, i) => i !== costIndex),
        };
      }),
    );
  }
  ...
  const maxLtv = (loanAmount, carPrice) =>
    carPrice > 0 ? (loanAmount / carPrice) * 100 : 0;
              ...
              <div className="px-6 pb-6 space-y-5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mb-1">
                    Nhập "Đến năm" để xác định khoảng thời gian áp dụng (để
                    trống = áp dụng mãi)
                  </p>
                  {bank.prepaymentFees.map((row, i) => (
                    <PrepaymentFeeRow
                      ...
                      onChange={(idx, updated) =>
                        updatePrepaymentFee(bank.id, idx, updated)
                      }
                ...
                <div>
                  ...
                  {bank.additionalCosts.map((cost, i) => (
                    <AdditionalCostRow
                      ...
                      onChange={(idx, updated) =>
                        updateAdditionalCost(bank.id, idx, updated)
                      }
