...
export default function ConclusionSection({ banks, scenarios, results }) {
  ...
  conclusions.forEach((c) => {
    if (c.best) winCount[c.best.bank.id] = (winCount[c.best.bank.id] || 0) + 1;
  });
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      {overallBest && (
        <div
          ...
          style={{
            borderColor: overallBest.color,
            backgroundColor: overallBest.color + "10",
          }}
        >
          <div className="flex items-center gap-2 mb-1">
            <div className="w-3 h-3 rounded-full" style={{ backgroundColor: overallBest.color }} />
            <span className="font-semibold text-slate-800">
              {overallBest.name} chiến thắng trong {winCount[overallBest.id]}/{scenarios.length}{" "}
              kịch bản
            </span>
          </div>
              ...
              <div className="flex-1">
                ...
                {!best.result.isFullTerm && best.result.prepaymentFee > 0 && (
                  <p className="text-xs text-slate-500 ml-4 mt-0.5">
                    Gồm phí tất toán {formatVND(best.result.prepaymentFee)} (
                    {best.result.prepaymentFeeRate}% dư nợ)
                  </p>
                )}
              </div>
              {others.map(({ bank, result }) => (
                <div key={bank.id} className="flex-1">
                  ...
                  <p className="text-xs text-red-500 ml-4 mt-0.5">
                    Đắt hơn +
                    {formatVND(result.costExcludingPrincipal - best.result.costExcludingPrincipal)}
                  </p>
                </div>
              ))}
            </div>
      ...
      {banks.length > 1 && (
        <div className="mt-5 pt-4 border-t border-slate-200">
          <h3 className="text-sm font-semibold text-slate-700 mb-3">
            Ngân hàng nào phù hợp với kịch bản nào?
          </h3>
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
            {banks.map((bank) => {
              const winScenarios = conclusions
                .filter((c) => c.best.bank.id === bank.id)
                .map((c) => c.scenario.label);
              const loseScenarios = conclusions
                .filter((c) => c.best.bank.id !== bank.id)
                .map((c) => c.scenario.label);
              return (
                <div
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    />
                    <span className="font-medium text-slate-700" style={{ color: bank.color }}>
                      {bank.name}
                    </span>
                  </div>
