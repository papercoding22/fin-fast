...
export default function DatePicker({ value, onChange, className = '' }) {
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      {open && (
        <div className="absolute z-50 mt-1.5 left-0 rounded-xl bg-white shadow-xl border border-slate-200 p-3 w-72">
          <DayPicker
            mode="single"
            selected={selected}
            ...
            classNames={{
              ...
              button_previous: 'p-1.5 rounded-lg hover:bg-slate-100 text-slate-500 hover:text-slate-800 transition-colors focus:outline-none',
              button_next: 'p-1.5 rounded-lg hover:bg-slate-100 text-slate-500 hover:text-slate-800 transition-colors focus:outline-none',
              weekdays: 'flex',
              weekday: 'w-9 h-8 flex items-center justify-center text-xs text-slate-400 font-medium',
              weeks: 'space-y-0.5',
              week: 'flex',
              day: 'flex items-center justify-center',
              day_button: 'w-9 h-9 rounded-lg text-sm flex items-center justify-center transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-blue-400 hover:bg-slate-100',
