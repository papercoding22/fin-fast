...
const CustomTooltipVND = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-lg text-xs">
      ...
      {payload.map((entry, i) => (
        <p key={i} style={{ color: entry.color }}>{entry.name}: {formatVND(entry.value)}</p>
      ))}
    </div>
  );
};
...
export function CostComparisonChart({ banks, scenarios, results }) {
  const data = scenarios.map(s => {
    const row = { name: s.label.replace('Tất toán sau ', 'TT ').replace('Giữ đến hết hạn', 'Hết hạn') };
    banks.forEach(b => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <h3 className="text-base font-semibold text-slate-800 mb-4">Chi phí thực (ngoài gốc) theo kịch bản</h3>
...
export function BalanceChart({ banks, schedules }) {
  ...
  const data = samplePoints.map(month => {
    ...
    banks.forEach(b => {
      ...
      if (!schedule) { row[b.name] = 0; return; }
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      <ResponsiveContainer width="100%" height={260}>
        <LineChart data={data} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
          ...
          <Legend />
          {banks.map(bank => (
            <Line key={bank.id} type="monotone" dataKey={bank.name} stroke={bank.color} dot={false} strokeWidth={2} />
          ))}
        </LineChart>
      </ResponsiveContainer>
    </div>
...
export function MonthlyPaymentChart({ banks, schedules }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <h3 className="text-base font-semibold text-slate-800 mb-4">Dòng tiền thanh toán hàng tháng</h3>
      <ResponsiveContainer width="100%" height={260}>
        <LineChart data={data} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
