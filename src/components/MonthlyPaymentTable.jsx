  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-100">
        <h2 className="text-lg font-semibold text-slate-800">Tiền trả hàng tháng tại các mốc quan trọng</h2>
        <p className="text-xs text-slate-500 mt-1">Phương pháp dư nợ giảm dần — gốc trả đều, lãi giảm dần</p>
      </div>
              ...
              return (
                ...
                >
                  <td className="px-4 py-2.5 text-slate-700 font-medium whitespace-nowrap">
                    ...
                    {isAfterFixed && <span className="ml-1 text-xs text-orange-600">(đầu thả nổi)</span>}
                    {month === loanTermMonths && <span className="ml-1 text-xs text-green-600">(tháng cuối)</span>}
                  </td>
                  {banks.map(bank => {
                    const schedule = schedules[bank.id];
                    if (!schedule) return <td key={bank.id} className="px-4 py-2.5 text-right text-slate-400">—</td>;
                    const payment = getPaymentAt(schedule, month);
                    if (!payment) return <td key={bank.id} className="px-4 py-2.5 text-right text-slate-400">—</td>;
                    return (
                      <td key={bank.id} className="px-4 py-2.5 text-right">
                        <div className="font-medium text-slate-800">{formatVND(payment.totalPayment)}</div>
